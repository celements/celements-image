...
public class GenerateThumbnail {
  ...
  public ImageDimensions getThumbnailDimensions(InputStream in, int maxWidth, int maxHeight,
      boolean lowerBounds, Color defaultBg) throws XWikiException {
  ...
  public ImageDimensions getThumbnailDimensions(BufferedImage img, int maxWidth, int maxHeight,
      boolean lowerBounds, Color defaultBg) {
    int width = maxWidth;
    ...
    return getThumbnailDimensions(width, height, maxWidth, maxHeight, lowerBounds, defaultBg);
  }
   ...
   * @param out
   *          OutputStream of the thumbnail.
   * @param width
   *          Maximum width of the thumbnail (Aspect ratio maintained).
   * @param height
   *          Maximum height of the thumbnail (aspect ratio maintained).
   * @param watermark
   *          String to add as a watermark to the image.
   * @param copyright
   *          String to add as a copyright to the image.
   * @return An ImageSize object representing the size of the thumbnail.
   * @throws IOException
   * @throws XWikiException
   */
  public ImageDimensions createThumbnail(InputStream in, OutputStream out, int width, int height,
      String watermark, String copyright, String type, Color defaultBg, boolean lowerBound,
      Integer lowerBoundPositioning, String overwriteType) throws IOException, XWikiException {
    return createThumbnail(decodeInputStream(in), out, width, height, watermark, copyright, type,
   ...
   *          OutputStream of the thumbnail.
   * @param dimensions
   *          Dimensions of the thumbnail (Aspect ratio maintained).
   * @param watermark
   *          String to add as a watermark to the image.
   * @param copyright
   *          String to add as a copyright to the image.
   ...
   * @throws XWikiException
   */
  public void createThumbnail(InputStream in, OutputStream out, ImageDimensions dimensions,
      String watermark, String copyright, String type, Color defaultBg, boolean lowerBound,
      Integer lowerBoundPositioning, String overwriteType) throws IOException, XWikiException {
    createThumbnail(decodeInputStream(in), out, dimensions, watermark, copyright, type, defaultBg,
        lowerBound, lowerBoundPositioning, overwriteType);
  }
   ...
   * @throws IOException
   */
  public ImageDimensions createThumbnail(BufferedImage img, OutputStream out, int width, int height,
      String watermark, String copyright, String type, Color defaultBg, boolean lowerBound,
      Integer lowerBoundPositioning, String overwriteType) throws IOException {
    ImageDimensions imgSize = getThumbnailDimensions(img, width, height, lowerBound, defaultBg);
  ...
  public BufferedImage createThumbnail(BufferedImage img, OutputStream out, ImageDimensions imgSize,
    ...
    if (lowerBound) {
      ...
      if ((img.getWidth() > (int) imgSize.getWidth())
          || (img.getHeight() > (int) imgSize.getHeight())) {
        LOGGER.trace("widthratio: {}, heightratio: {}", (img.getWidth() / imgSize.getWidth()),
        ...
        if ((img.getWidth() / imgSize.getWidth()) > (img.getHeight() / imgSize.getHeight())) {
          ...
          thumbImg = img.getScaledInstance(-1, (int) imgSize.getHeight(), Image.SCALE_SMOOTH);
        }
      }
    }
  ...
  int getLowerBoundFinalPositioning(int pos, int baseLength, int frameLength) {
    ...
    if (pos < 0) {
      LOGGER.debug("Positioning: pos=" + pos + ", base=" + baseLength + ", frame=" + frameLength
          + " positioning=" + (-pos - (baseLength - frameLength)));
   ...
   * @param image
   *          BufferedImage of the image to encode.
   * @param fallback
   *          BufferedImage to fall back if encoding of image fails.
   * @param type
   *          Mime type of the input image.
   * @param overrideType
   *          Mime type of the output image.
   */
  public void encodeImage(OutputStream out, CelImage image, CelImage fallback, String type,
      String overrideType) {
    LOGGER.info("endoceImage: type [{}], override type [{}], override in allowed types " + "[{}]",
        type, overrideType, saveTypes.containsKey(Strings.isNullOrEmpty(overrideType) ? ""
            : overrideType.toLowerCase()));
    ...
    LOGGER.info("encodeImage: get ImageWriter for format name [{}], writer [{}]", saveTypes.get(
        type.toLowerCase()), writer);
    ...
    if (imgOut != null) {
      ...
      try {
        ...
        try {
          outImage = new IIOImage(fallback.getFirstImage(), null, fallback.getFirstMetadata());
          writer.write(null, outImage, writer.getDefaultWriteParam());
        } catch (IOException e) {
  ...
  public void encodeImage(OutputStream out, BufferedImage image, BufferedImage fallback,
    ...
    boolean forcePng = Strings.isNullOrEmpty(overrideType) && !"png".equals(type.toLowerCase());
  ...
  private void drawWatermark(String watermark, Graphics2D g2d, int width, int height) {
    // TODO implement 'secure' watermark (i.e. check if this algorithm secure or can it be easily
    // reversed?)
    ...
    g2d.drawString(watermark, (width - metrics.stringWidth(watermark)) / 2, ((height
  ...
  private void drawBackground(String copyright, int width, int height, int bottomSpace,
      int rightSpace, int vSpacing, int hSpacing, int rounding, int stringHeight,
  ...
  private void drawString(String copyright, int width, int height, int bottomSpace, int rightSpace,
      int vSpacing, int hSpacing, FontMetrics metrics, Graphics2D g2d) {
    ...
    g2d.drawString(copyright, width - metrics.stringWidth(copyright) - hSpacing - rightSpace, height
        - metrics.getDescent() - vSpacing - bottomSpace);
  }
