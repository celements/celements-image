...
public class CelementsPhotoPlugin extends XWikiDefaultPlugin {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(CelementsPhotoPlugin.class);

  /**
   * The image formats supported by the image plugin
   */
  public enum SupportedFormat {
    JPG(1, "image/jpg"), JPEG(1, "image/jpeg"), PNG(2, "image/png"), GIF(3, "image/gif"), BMP(4,
        "image/bmp"),
  ...
  public XWikiAttachment downloadAttachment(XWikiAttachment attachment, XWikiContext context) {
    long delayDownloadMillis = context.getWiki().getXWikiPreferenceAsLong("debugDelayDownload",
        "cel.debug.delay.download", 0, context);
    if (delayDownloadMillis > 0) {
      LOGGER.warn("downloadAttachment: debugging sleep activated for {}", delayDownloadMillis);
      try {
        Thread.sleep(delayDownloadMillis);
      } catch (InterruptedException exp) {
        LOGGER.error("downloadAttachment: test sleep", exp);
      }
    }
    if (this.isSupportedImageFormat(attachment.getMimeType(context))) {
      ...
      String attSpace = attachment.getDoc().getDocumentReference().getLastSpaceReference().getName();
      String filterString = context.getWiki().getSpacePreference("imageDownloadFilter", attSpace,
          "", context);
      if ((filterString == null) || "".equals(filterString)) {
        filterString = context.getRequest().getParameter("filter");
      }
      String mimeType = context.getRequest().getParameter("mimeType");
      return getComputeImgCmd().computeImage(attachment, context, attachment, sheight, swidth,
   ...
   * @param width
   *          Desired maximum width of the thumbnails (aspect ratio is maintained).
   * @param height
   *          Desired maximum height of the thumbnails (aspect ratio is maintained).
   * @param context
   *          XWikiContext
   ...
   * @throws IOException
   */
  public ImageStrings[] getImageList(XWikiDocument doc, int width, int height, XWikiContext context)
   ...
   * @throws XWikiException
   */
  public boolean isImageDeleted(XWikiDocument doc, String id, XWikiContext context)
      throws XWikiException {
   ...
   *          Id of the image.
   * @param deleted
   *          true to tag the image as deleted.
   * @param XWikiContext
   * @throws XWikiException
   */
  public void setImageDeleted(XWikiDocument doc, String id, boolean deleted, XWikiContext context)
      throws XWikiException {
   ...
   * @throws IOException
   */
  public String getThumbnailUrl(XWikiDocument doc, String id, int width, int height,
   ...
   * @throws IOException
   */
  public void forceClearMetadata(XWikiDocument doc, String id, XWikiContext context)
      throws XWikiException, IOException {
  ...
  public List<ImportFileObject> getAttachmentFileListWithActions(XWikiAttachment importFile,
      XWikiDocument galleryDoc, XWikiContext context) throws XWikiException {
      ...
      if (isZipFile(importFile, context)) {
        ...
        try {
          fileList = (new Unzip()).getZipContentList(IOUtils.toByteArray(
              importFile.getContentInputStream(context)));
        ...
        ImportFileObject file = new ImportFileObject(importFile.getFilename(), getActionForFile(
            importFile.getFilename(), galleryDoc, context));
        resultList.add(file);
      }
    } else {
   ...
   * 
   * @param zipFile
   *          File containing the image to extract.
   * @param unzipFileName
   *          Filename of the image to extract.
   * @param attachToDoc
   *          Document to attach the extracted and resized image.
   * @param width
   *          Width (max - aspect ratio is maintained) to resize the image to.
   * @param height
   *          Height (max - aspect ratio is maintained) to resize the image to.
   * @param context
   *          XWikiContezt
   * @throws XWikiException
   */
  ...
  public void unzipFileToAttachment(XWikiAttachment zipFile, String unzipFileName,
    ...
    try {
      if (isZipFile(zipFile, context)) {
        ...
        imgFullSize = new ByteArrayInputStream(IOUtils.toByteArray(zipFile.getContentInputStream(
            context)));
      }
      ...
      ImageDimensions id = (new GenerateThumbnail()).createThumbnail(imgFullSize, out, width,
          height, null, null, mimeType, null, false, null, null);
      ...
      LOGGER.info("attachment='" + att.getFilename() + "', gallery='"
          + att.getDoc().getDocumentReference() + "' size='" + att.getFilesize() + "'");
  ...
  private boolean isZipFile(XWikiAttachment file, XWikiContext context) {
    return file.getMimeType(context).equalsIgnoreCase(ImageLibStrings.MIME_ZIP) || file.getMimeType(
        context).equalsIgnoreCase(ImageLibStrings.MIME_ZIP_MICROSOFT);
  }
