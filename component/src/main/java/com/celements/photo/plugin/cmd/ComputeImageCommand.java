...
public class ComputeImageCommand {
  ...
  public XWikiAttachment computeImage(XWikiAttachment attachment, XWikiContext context,
      XWikiAttachment attachmentClone, String sheight, String swidth, String copyright,
      String watermark, Color defaultBg, String defaultBgStr, String filterStr,
    ...
    LOGGER.debug("Crop needed: " + needsCropping + " -> " + cropX + ":" + cropY + " " + cropW + "x"
        + cropH);
    ...
    boolean lowerBound = 1 == parseIntWithDefault(context.getRequest().get("lowBound"), 0);
    Integer lowerBoundPositioning = parseIntWithDefault(context.getRequest().get("lowBoundPos"),
        null);
    ...
    try {
      ...
      String key = getImageCacheCmd().getCacheKey(attachmentClone, new ImageDimensions(width,
          height), copyright, watermark, cropX, cropY, cropW, cropH, blackAndWhite, defaultBg,
          lowerBound, lowerBoundPositioning, filterStr, overwriteOutputFormat, raw);
      ...
      if (data != null) {
        ...
        if (!raw) {
          ...
          if ((height > 0) || (width > 0)) {
            // TODO accept CelImage instead of bufferedImage
            ImageDimensions dimension = thumbGen.getThumbnailDimensions(img.getFirstImage(), width,
                height, lowerBound, defaultBg);
            timeLast = logRuntime(timeLast, "got image dimensions after ");
            // TODO accept CelImage instead of bufferedImage
            byte[] thumbImageData = getThumbAttachment(img.getFirstImage(), dimension, thumbGen,
                mimeType, null, watermark, copyright, defaultBg, lowerBound, lowerBoundPositioning);
          ...
          if ((filterStr != null) && !"".equals(filterStr)) {
            ...
            if (filterParts.length > 2) {
              try {
                ...
                LOGGER.debug("Filtering with kernel configured as " + kerWidth + ", " + kerHeight
                    + ", " + Arrays.toString(kerMatrix));
                ...
                attachmentClone.setContent(new ByteArrayInputStream(out.toByteArray()));
                timeLast = logRuntime(timeLast, "applied kernel filter [" + filterStr + "] after ");
          ...
          if (outFormatChange) {
            ...
            ByteArrayOutputStream out = new ByteArrayOutputStream();
            (new GenerateThumbnail()).encodeImage(out, img, img, mimeType, overwriteOutputFormat);
            attachmentClone.setContent(new ByteArrayInputStream(out.toByteArray()));
  ...
  void setBorderPixelsBeforeNeededForKernel(int xOffset, int yOffset, BufferedImage newSource,
      BufferedImage img) {
    // set top left corner
    ...
    for (int i = 0; i < img.getWidth(); i++) {
      ...
      for (int k = 0; k < yOffset; k++) {
        ...
        newSource.setRGB(x, (newSource.getHeight() - 1) - k, img.getRGB(i, (img.getHeight() - 1)));
      }
    }
    // set top right corner
    for (int i = 0; i < xOffset; i++) {
      for (int k = 0; k < yOffset; k++) {
        newSource.setRGB((newSource.getWidth() - 1) - i, k, img.getRGB((img.getWidth() - 1), 0));
      }
    }
    // set bottom left corner
    for (int i = 0; i < xOffset; i++) {
      for (int k = 0; k < yOffset; k++) {
        newSource.setRGB(i, (newSource.getHeight() - 1) - k, img.getRGB(0, (img.getHeight() - 1)));
      }
    }
    ...
    for (int i = 0; i < img.getHeight(); i++) {
      ...
      for (int k = 0; k < xOffset; k++) {
        ...
        newSource.setRGB((newSource.getWidth() - 1) - k, y, img.getRGB((img.getWidth() - 1), i));
      }
    }
    // set bottom right corner
    for (int i = 0; i < xOffset; i++) {
      for (int k = 0; k < yOffset; k++) {
        newSource.setRGB((newSource.getWidth() - 1) - i, (newSource.getHeight() - 1) - k,
  ...
  Color getBackgroundColour(Color defaultBg, String defaultBgStr) {
    if ((defaultBgStr != null) && defaultBgStr.matches("[0-9A-Fa-f]{6}([0-9A-Fa-f]{2})?")) {
  ...
  private byte[] getThumbAttachment(BufferedImage img, ImageDimensions dim,
      GenerateThumbnail thumbGen, String mimeType, String overwriteOutputFormat, String watermark,
      String copyright, Color defaultBg, boolean lowerBound, Integer lowerBoundPositioning) {
    ByteArrayOutputStream out = new ByteArrayOutputStream();
    thumbGen.createThumbnail(img, out, dim, watermark, copyright, mimeType, defaultBg, lowerBound,
        lowerBoundPositioning, overwriteOutputFormat);
