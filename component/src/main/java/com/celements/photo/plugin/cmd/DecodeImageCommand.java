...
public class DecodeImageCommand {
  ...
  public CelImage readImage(XWikiAttachment att, XWikiContext context) throws ImageReadException,
      XWikiException {
    return readImage(att.getContentInputStream(context), att.getFilename(), att.getMimeType(
        context));
  }

  public CelImage readImage(InputStream imageStream, String filename, String mimeType)
  ...
  BufferedImage convertCmykToRgb(Raster cmykRaster, ICC_Profile cmykProfile) throws IOException {
  ...
  ICC_Profile getICCProfile(ICC_Profile cmykProfile) throws IOException {
    if (cmykProfile == null) {
      cmykProfile = ICC_Profile.getInstance(getClass().getClassLoader().getResourceAsStream(
          getCMYKProfile()));
    }
