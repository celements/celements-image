...
public class ZipAttachmentChanges {
  ...
  public void checkZipAttatchmentChanges(XWikiDocument doc, XWikiContext context)
    ...
    DocumentReference albumRef = new DocumentReference(context.getDatabase(),
        ImageLibStrings.getPhotoSpace(doc), doc.getDocumentReference().getName() + "_alb_");
    ...
    if (savedAttachmentList != null) {
      ...
      removeDataFromRemovedZip(savedAttachmentList, notDeletedAttachments, doc, albumDoc, context);
    }
  ...
  private void deleteImagesFromZip(BaseObject element, boolean delete, XWikiDocument doc,
      XWikiDocument albumDoc, XWikiContext context) throws XWikiException {
    if (delete) {
      String filename = "";
      List<String> fileList = context.getWiki().getSpaceDocsName(ImageLibStrings.getPhotoSpace(doc),
          context);
      if (fileList != null) {
        Iterator<String> iterator = fileList.iterator();
        while (iterator.hasNext() && !filename.equals(doc.getDocumentReference().getName() + "_zip_"
            + getFilenameHash(element.getStringValue(
  ...
  private void deleteThumbAndMetaForAll(BaseObject element, XWikiDocument doc, String filename,
      XWikiDocument albumDoc, XWikiContext context) throws XWikiException {
  ...
  private void updateOrDeleteMetaData(String imageHash, XWikiDocument doc, XWikiContext context)
      throws XWikiException {
    ...
    if (revision <= 1) {
      ...
      handler.setImageInteger(celementsMetaDoc, ImageLibStrings.PHOTO_IMAGE_REVISION, revision - 1,
          context);
      ...
      for (Iterator<XWikiAttachment> attIter = attachmentList.iterator(); attIter.hasNext();) {
        att = attIter.next();
        DocumentReference zipDocRef = new DocumentReference(context.getDatabase(),
            ImageLibStrings.getPhotoSpace(doc), doc.getDocumentReference().getName() + "_zip_"
                + getFilenameHash(att.getFilename()));
  ...
  private void setNameAndDirectory(String filenameInMeta, XWikiAttachment att,
      BaseObjectHandler handler, XWikiDocument celementsMetaDoc, XWikiContext context)
      throws XWikiException {
    // ==null might occur when there is no existing version anymore and has not been removed
    // correctly
    if (filenameInMeta != null) {
      ...
      handler.setImageString(celementsMetaDoc, ImageLibStrings.PHOTO_IMAGE_ZIPDIRECTORY, dir,
          context);
  ...
  private void getImagesFromZip(XWikiAttachment element, boolean added, XWikiDocument doc,
      XWikiDocument albumDoc, XWikiContext context) throws XWikiException, IOException {
    String mimeType = element.getMimeType(context);

    if (added && ((mimeType.equalsIgnoreCase(ImageLibStrings.MIME_ZIP) || mimeType.equalsIgnoreCase(
      ...
      for (Iterator<String> iterator = hashes.iterator(); iterator.hasNext();) {
        ...
        handler.addBaseObject(zipMetaDoc, ImageLibStrings.METATAG_IMAGE_HASH, hash, context);
        handler.addBaseObject(zipMetaDoc, hash, dir + filename, context);
      }

      handler.addBaseObject(albumDoc, ImageLibStrings.METATAG_ZIP_FILENAME, element.getFilename(),
          context);
    }
  }
  ...
  private XWikiDocument writeHashesToMetaDoc(XWikiAttachment element, XWikiDocument doc,
      XWikiContext context) throws XWikiException {
    DocumentReference metaDocRef = new DocumentReference(context.getDatabase(),
        ImageLibStrings.getPhotoSpace(doc), doc.getDocumentReference().getName() + "_zip_"
            + getFilenameHash(element.getFilename()));
  ...
  private int updateMetaObject(XWikiAttachment element, String dir, String filename,
      BaseObjectHandler handler, XWikiDocument imgMetaDoc, XWikiContext context)
      throws XWikiException {
    int revision = handler.getImageInteger(imgMetaDoc, ImageLibStrings.PHOTO_IMAGE_REVISION);
    handler.setImageInteger(imgMetaDoc, ImageLibStrings.PHOTO_IMAGE_REVISION, revision + 1,
        context);
    handler.setImageString(imgMetaDoc, ImageLibStrings.PHOTO_IMAGE_ZIPNAME, element.getFilename(),
        context);
    handler.setImageString(imgMetaDoc, ImageLibStrings.PHOTO_IMAGE_ZIPDIRECTORY, dir, context);
    handler.setImageString(imgMetaDoc, ImageLibStrings.PHOTO_IMAGE_FILENAME, filename, context);
    return revision;
  }
   ...
   * @throws IOException
   */
  public InputStream getFromZip(XWikiDocument album, String image, String id, XWikiContext context)
      throws XWikiException, IOException {
   ...
   * @throws XWikiException
   */
  public XWikiAttachment getContainingZip(XWikiDocument doc, String id, XWikiContext context)
      throws XWikiException {
    DocumentReference metaDocRef = new DocumentReference(context.getDatabase(),
  ...
  public InputStream getFromZip(XWikiAttachment zip, String image, XWikiContext context)
    ...
    if (image != null) {
      byte[] imageArray;
      imageArray = (new Unzip()).getFile(IOUtils.toByteArray(zip.getContentInputStream(context)),
          image).toByteArray();
