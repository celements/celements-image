...
public class Metainfo {
  ...
  public Metadate getTag(XWikiDocument doc, String id, String tag, XWikiContext context)
    ...
    for (int i = 0; i < metainfos.size(); i++) {
      ...
      if ((metadate != null) && (metadate.getStringValue(
        ...
        metatag = new Metadate(metadate.getStringValue(ImageLibStrings.METAINFO_CLASS_NAME),
            metadate.getStringValue(ImageLibStrings.METAINFO_CLASS_DESCRIPTION));
        break;
      }
    }
  ...
  private void getAvailableNotExcludedTags(String conditionTag, boolean condition,
    ...
    for (Iterator<BaseObject> iter = metainfos.iterator(); iter.hasNext();) {
      ...
      if ((metadate != null) && (condition || !metadate.getStringValue(
        ...
        resultList.add(new Metadate(metadate.getStringValue(ImageLibStrings.METAINFO_CLASS_NAME),
            metadate.getStringValue(ImageLibStrings.METAINFO_CLASS_DESCRIPTION)));
      }
    }
  }
  ...
  public List<BaseObject> getMetadataList(XWikiDocument doc, String id, XWikiContext context)
    ...
    XWikiDocument metaDoc = context.getWiki().getDocument(ImageLibStrings.getPhotoSpace(doc),
        doc.getName() + "_img_" + id, context);
    List<BaseObject> metainfos = metaDoc.getXObjects(ImageLibStrings.getMetainfoClassDocRef());
  ...
  public void extractMetaToDoc(XWikiDocument doc, String id, XWikiContext context)
    ...
    XWikiDocument celeMeta = context.getWiki().getDocument(ImageLibStrings.getPhotoSpace(doc), album
        + "_img_" + id, context);
    ...
    List<BaseObject> metadataObjs = celeMeta.getXObjects(ImageLibStrings.getMetainfoClassDocRef());
  ...
  private void initNewObject(XWikiDocument celeMeta, String key, String value, XWikiContext context)
      throws XWikiException {
    BaseObject metainfoObj = celeMeta.newXObject(ImageLibStrings.getMetainfoClassDocRef(), context);
   ...
   * @throws IOException
   */
  private Map<String, String> extractAllMetadata(XWikiDocument albumDoc, String image, String id,
      XWikiContext context) throws MetadataException, XWikiException, IOException {
    InputStream imgAttachment = (new ZipAttachmentChanges()).getFromZip(albumDoc, image, id,
        context);
   ...
   *          XWikiDocument of the album.
   * @param image
   *          Name of the image the metadata belongs to.
   * @param imageMethods
  ...
  private void clearOutdatedMetadata(XWikiDocument doc, String id, XWikiContext context)
    ...
    XWikiDocument metadataDoc = context.getWiki().getDocument(ImageLibStrings.getPhotoSpace(doc),
        doc.getName() + "_img_" + id, context);

    if (!metadataDoc.isNew()) {
      XWikiAttachment containingZip = (new ZipAttachmentChanges()).getContainingZip(doc, id,
          context);
