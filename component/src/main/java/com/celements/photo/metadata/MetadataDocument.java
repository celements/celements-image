...
public class MetadataDocument implements IMetadataDocumentRole {
  ...
  public void extractMetadataToDocument(DocumentReference source, String filename,
    ...
    try {
      ...
      Map<String, String> tags = extractor.getAllTags(file.getContentInputStream(getContext()));

      XWikiDocument destinationDoc = getContext().getWiki().getDocument(destination, getContext());
      ...
      LOGGER.error("Exception extracting metadata for file [" + source + " - " + filename + "]",
          metaE);
  ...
  public Boolean addTagsToDoc(Map<String, String> tags, XWikiDocument destinationDoc,
    ...
    for (String tag : tags.keySet()) {
      if ((filteredImport == false) || !tag.matches("Unknown .*")) {
        ...
        needsSave = true;
        BaseObject tagObj = destinationDoc.newXObject(ImageLibStrings.getMetainfoClassDocRef(),
            getContext());
