...
public class UnpackComponent implements IUnpackComponentRole {
  ...
  public String unzipFileToAttachment(DocumentReference zipSrcDocRef, String attachmentName,
    ...
    String cleanName = "";
    try {
      XWikiDocument zipSourceDoc = getContext().getWiki().getDocument(zipSrcDocRef, getContext());
    ...
    if (zipSrcFile != null) {
      ...
      try {
        ...
        if (isZipFile(zipSrcFile)) {
          newAttOutStream = getUnzip().getFile(attName, zipSrcFile.getContentInputStream(
              getContext()));
          ...
          imageContent = IOUtils.toByteArray(zipSrcFile.getContentInputStream(getContext()));
        ...
        if (imageContent != null) {
          XWikiDocument destDoc = getContext().getWiki().getDocument(destDocRef, getContext());
          XWikiAttachment att = attService.addAttachment(destDoc, imageContent, cleanName,
              getContext().getUser(), null);
          LOGGER.info("attachment='" + att.getFilename() + "', doc='"
      ...
      LOGGER.error("Source document which should contain zip file is null: [" + zipSrcFile + "]");
    }
  ...
  boolean isImgFile(String fileName) {
    return (fileName != null) && (fileName.toLowerCase().endsWith("." + ImageLibStrings.MIME_BMP)
